...
type RequestBody = { model?: string; messages?: Message[] };
...
async function unipool(model: string, messages: Message[]) {
  ...
  if (!key) return error("UniPool is not configured. Add UNIPOOL_API_KEY to the Vercel environment.", 503);
  try {
    const response = await fetch(`${UNIPOOL_BASE_URL}/chat/completions`, {
      method: "POST",
    ...
    if (!response.ok) return error(data?.error?.message || data?.error || `UniPool returned ${response.status}`, response.status);
    const content = data?.choices?.[0]?.message?.content ?? data?.choices?.[0]?.text ?? "";
    return NextResponse.json({ content, provider: "unipool", model, usage: data?.usage ?? null });
  } catch {
    return error("Unable to reach UniPool.");
  }
}

async function literouter(model: string, messages: Message[]) {
  const key = process.env.LITEROUTER_API_KEY;
  if (!key) return error("LiteRouter is not configured.", 503);
  try {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) return error(data?.error?.message || `LiteRouter returned ${response.status}`, response.status);
    return NextResponse.json({ content: data?.choices?.[0]?.message?.content ?? "", provider: "literouter", model, usage: data?.usage ?? null });
  } catch {
    return error("Unable to reach LiteRouter.");
  }
}
...
async function horde(model: string, messages: Message[]) {
  ...
  try {
    ...
    if (!submit.ok) return error(`AI service returned ${submit.status}`, submit.status);
    const job = await submit.json() as { id?: string };
    if (!job.id) return error("The AI service did not return a job.");
    for (let i = 0; i < 60; i++) {
      ...
      if (data.faulted) return error("The generation failed.");
  ...
  } catch { return error("Unable to reach the AI service."); }
}

export async function POST(req: Request) {
  const body = await req.json().catch(() => null) as RequestBody | null;
  if (!body?.model || !Array.isArray(body.messages) || body.messages.length === 0) return error("A model and messages are required.", 400);
  if (body.model.includes(":" ) && LITEROUTER_MODELS.has(body.model)) return literouter(body.model, body.messages);
  const uniKey = process.env.UNIPOOL_API_KEY;
  if (uniKey) {
    try {
      const r = await fetch(`${UNIPOOL_BASE_URL}/models`, { headers: { Authorization: `Bearer ${uniKey}` }, cache: "no-store" });
      if (r.ok) {
